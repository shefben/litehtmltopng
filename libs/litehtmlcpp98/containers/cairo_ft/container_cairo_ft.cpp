...
container_cairo_ft::container_cairo_ft()
{
	m_temp_surface	= cairo_image_surface_create(CAIRO_FORMAT_ARGB32, 2, 2);
	m_temp_cr		= cairo_create(m_temp_surface);
	const FT_Error ft_status = FT_Init_FreeType(&m_ft_library);
	if (ft_status != 0) {
		fprintf(stderr, "Error %d opening FreeType library\n", ft_status);
		m_ft_library = 0;//ft_library is no available
	}
}

container_cairo_ft::~container_cairo_ft(void)
{
	clear_images();
	cairo_surface_destroy(m_temp_surface);
	cairo_destroy(m_temp_cr);
	if (ft_library_available()) {
		FT_Done_FreeType(m_ft_library);
	}
}

bool container_cairo_ft::ft_library_available() const
{
	return m_ft_library != 0 ? true : false;
}

litehtml::uint_ptr container_cairo_ft::create_font( const litehtml::tchar_t* faceName, int size, int weight, litehtml::font_style italic, unsigned int decoration, litehtml::font_metrics* fm )
{
	if (!ft_library_available()) {
	...
	//const char * filename = faceName;//example: "arial.ttf";
	const char * filename =
		"D:\\Repos\\ReceiptTemplate\\litehtml\\arial.ttf";

	FT_Face ft_face;//FT_New_Face out face
	const FT_Error ft_status = FT_New_Face(m_ft_library, filename, 0, &ft_face);
	if (ft_status != 0) {
	...
	if (fm && fnt)
	{
		...
		ret = new cairo_font();//remove in method delete_font
		ret->font = fnt;
		ret->size = size;
...
void container_cairo_ft::delete_font( litehtml::uint_ptr hFont )
{
...
int container_cairo_ft::text_width( const litehtml::tchar_t* text, litehtml::uint_ptr hFont )
{
	cairo_font* fnt = (cairo_font*) hFont;
...
void container_cairo_ft::draw_text( litehtml::uint_ptr hdc, const litehtml::tchar_t* text, litehtml::uint_ptr hFont, litehtml::web_color color, const litehtml::position& pos )
{
	cairo_font* fnt = (cairo_font*) hFont;
...
int container_cairo_ft::pt_to_px( int pt )
{
	litehtml::media_features media;
	get_media_features(media);
	return (int)((double)pt * media.resolution / 72.0);
}

int container_cairo_ft::get_default_font_size() const
{
	return 16;
}

void container_cairo_ft::draw_list_marker( litehtml::uint_ptr hdc, const litehtml::list_marker& marker )
{
...
void container_cairo_ft::load_image( const litehtml::tchar_t* src, const litehtml::tchar_t* baseurl, bool redraw_on_ready )
{
	litehtml::tstring url;
	...
	if(m_images.find(url.c_str()) == m_images.end())
	{
		try
		{
			image_ptr img = get_image(url.c_str(), true);
			if(img)
			{
...
void container_cairo_ft::get_image_size( const litehtml::tchar_t* src, const litehtml::tchar_t* baseurl, litehtml::size& sz )
{
	litehtml::tstring url;
	make_url(src, baseurl, url);
...
void container_cairo_ft::draw_background( litehtml::uint_ptr hdc, const litehtml::background_paint& bg )
{
	cairo_t* cr = (cairo_t*) hdc;
	cairo_save(cr);
	apply_clip(cr);
	...
	if(img_i != m_images.end() && img_i->second)
	{
		...
		//code from litehtml\containers\linux\container_linux.cpp method container_linux::draw_background
		cairo_format_t format = cairo_image_surface_get_format(bgbmp->surface());
		cairo_surface_t* img = cairo_image_surface_create(format, bgbmp->get_width(), bgbmp->get_height());
		...
		case litehtml::background_repeat_no_repeat:
			...
			cairo_translate(cr, bg.position_x, bg.position_y);//offset px
			cairo_scale(cr, double(bg.image_size.width) / bgbmp->get_width(), double(bg.image_size.height) / bgbmp->get_height());
			cairo_set_source_surface(cr, bgbmp->surface(), 0, 0);
			cairo_paint(cr);
			break;
...
container_cairo_ft::image_ptr container_cairo_ft::get_image(const litehtml::tchar_t* url, bool redraw_on_ready)
{
	cairo_surface_t* image_surface = cairo_image_surface_create_from_png(url);
	cairo_status_t status = cairo_surface_status(image_surface);
	container_cairo_ft::image_ptr ret;
	if (CAIRO_STATUS_SUCCESS == status) {
		ret = container_cairo_ft::image_ptr(new cario_image(image_surface));
	}
	else {
		fprintf(stderr, "Error image url=%s\n", url);
	}
	return ret;
}

void container_cairo_ft::add_path_arc(cairo_t* cr, double x, double y, double rx, double ry, double a1, double a2, bool neg)
{
	if(rx > 0 && ry > 0)
	{
...
void container_cairo_ft::draw_borders(litehtml::uint_ptr hdc, const litehtml::borders& borders, const litehtml::position& draw_pos, bool root)
{
	cairo_t* cr = (cairo_t*) hdc;
	cairo_save(cr);
	apply_clip(cr);
...
void container_cairo_ft::transform_text(litehtml::tstring& text, litehtml::text_transform tt)
{

}

void container_cairo_ft::set_clip( const litehtml::position& pos, const litehtml::border_radiuses& bdr_radius, bool valid_x, bool valid_y )
{
...
void container_cairo_ft::del_clip()
{
	if(!m_clips.empty())
	{
		m_clips.pop_back();
...
void container_cairo_ft::apply_clip( cairo_t* cr )
{
...
void container_cairo_ft::draw_ellipse( cairo_t* cr, int x, int y, int width, int height, const litehtml::web_color& color, int line_width )
{
	if(!cr) return;
	cairo_save(cr);
...
void container_cairo_ft::fill_ellipse( cairo_t* cr, int x, int y, int width, int height, const litehtml::web_color& color )
{
	if(!cr) return;
	cairo_save(cr);
...
void container_cairo_ft::clear_images()
{
...
const litehtml::tchar_t* container_cairo_ft::get_default_font_name() const
{
...
void container_cairo_ft::rounded_rectangle( cairo_t* cr, const litehtml::position &pos, const litehtml::border_radiuses &radius )
{
	cairo_new_path(cr);
...
void container_cairo_ft::get_language(litehtml::tstring& language, litehtml::tstring& culture) const
{
	language = _t("en");
	culture = _t("");
}

...
litehtml::el_before_after_base::el_before_after_base(const lhmemory_shared_ptr<litehtml::document>& doc, bool before) : html_tag(doc)
{
	if(before)
	{
	...
	for(tstring::size_type i = 0; i < txt.length(); i++)
	{
		if( (txt.at(i) == _t(' ')) || (txt.at(i) == _t('\t')) || (txt.at(i) == _t('\\') && !esc.empty()) )
		{
			if(esc.empty())
			{
				if(!word.empty())
				{
					element::ptr el = lhmemory_make_shared<el_text>(word.c_str(), get_document());
					appendChild(el);
					word.clear();
				}

				element::ptr el = lhmemory_make_shared<el_space>(txt.substr(i, 1).c_str(), get_document());
				appendChild(el);
			} else
			{
	...
	case 2:
		{
			...
			if(!p_url.empty())
			{
				element::ptr el = lhmemory_make_shared<el_image>(get_document());

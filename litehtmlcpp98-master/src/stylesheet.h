...
namespace litehtml
{
	...
	public:
		...
		void	parse_stylesheet(const tchar_t* str, const tchar_t* baseurl, const lhmemory_shared_ptr <document>& doc, const media_query_list::ptr& media);
		void	sort_selectors();
		static void	parse_css_url(const tstring& str, tstring& url);

	private:
		void	parse_atrule(const tstring& text, const tchar_t* baseurl, const lhmemory_shared_ptr<document>& doc, const media_query_list::ptr& media);
